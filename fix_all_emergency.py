...
def fix_all_problematic():
    ...
    for filename, city_name in problem_files.items():
        ...
        print(f"--- İŞLENİYOR: {city_name} ({filename}) ---")
        ...
        for i, row in enumerate(ws.iter_rows(values_only=True, max_row=5000)):
            ...
            # 1. Hastane Tespiti (Reference Matching)
            # Alt alta olan dosyalarda genelde ilk hücrede hastane adı geçer
            if len(val) > 10 and ('HASTANE' in val_up or 'MERKEZ' in val_up or 'ADSM' in val_up):
            ...
            # 2. Veri Kaydı
            if current_hosp_name and len(val) > 20:
    ...
    conn.commit()
    print(f"\nİŞLEM TAMAMLANDI. TOPLAM: {total_count} kayıt.")
    cur.close()
    conn.close()
